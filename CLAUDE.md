...
Backend of an Internal Developer Platform (IDP). Provides JWT-based authentication, OAuth integration (GitHub/GitLab), a repository catalog with GitHub sync, pull request and issue browsing with review actions, spatial repository relationship mapping, CI coverage ingestion, and AI-generated project documentation delivered as GitHub PRs.
...
7. **Pull Request Browsing**: Complete — `PullRequestHandler` proxying the repository's own host (GitHub pull requests or GitLab merge requests): list, detail, and changed files with patches, plus the two review verdicts (approve, request changes). Routes and DTOs keep saying "pull request"; the neutral term "change request" is internal to `internal/integrations/scm`. No queue, no cache, no analysis.
...
14. **Issues & Contributors**: Complete — `RepositoryBrowseHandler` lists open issues and contributors for a repository and closes an issue, sharing repository→provider resolution with `PullRequestHandler` through the unexported `scmResolver`. Two things are deliberate: GitHub's `GET /issues` returns pull requests too and the client filters them out, and a contributor's change-request count and last activity are *derived* (from the change-request list and the recent-commit window) because neither provider reports them — they are nullable so "could not determine" never renders as zero.
15. **Host write actions carry the organization's identity, not the user's**: approving a change request or closing an issue is authenticated with the organization's configured token (`organization_configs.github_token` / `gitlab_token`), because OAuth here requests only `read:user`. The host therefore attributes the action to whoever owns that token — on GitHub an approval under that name can also satisfy branch protection. `reviewBody` stamps the real actor into the review message so the record is not silently wrong. When the host refuses because that identity authored the change request, the refusal is reported as **409 `self_review`** naming the token's owner — never 503, which would invite a retry that cannot succeed. That 409 is **correct behaviour, not a defect**: nobody may approve their own change request, and the token owner is the author whenever they opened it. It looks like a total block on a single-person deployment, where the same person opens everything; change requests from anyone else (Dependabot, say) approve normally. `GET /pull-requests/:n` also carries `review_blocked_reason` so a client can refuse before the click; it is advisory and costs no extra request, while the token owner is resolved via `scm.IdentityReader` only after a rejection has happened. Fixing this properly still needs per-user write scopes.
16. **Review state is read, not assumed**: `scm.ChangeRequestReviewReader` reports the recorded verdict (`GET /pulls/{n}/reviews` on GitHub, `GET /merge_requests/:iid/approvals` on GitLab), surfaced as `review_decision` / `approved_by` / `changes_requested_by`. Three things are load-bearing. GitHub returns a review *history*, so the adapter keeps only each reviewer's latest position and treats `DISMISSED` as withdrawing it; `changes_requested` outranks `approved` in the summary. GitLab has no changes-requested state at all, so its list is always empty there — the same asymmetry `RequestChanges` reports as `ErrUnsupportedCapability`. And `review_decision` is **null when the host could not be asked** versus `""` for a measured "nobody reviewed"; collapsing the two would render an unknown as "not reviewed". Neither host reports this on its list endpoint, so the list handler pays one request per change request, bounded by `reviewStateFanOut` (concurrency) and `reviewStateListCeiling` (how many are attempted at all); change requests past the ceiling report null and the skip is logged.
...
- **Providers go through `internal/integrations/scm`**: `SyncService`, `DocsWorker` and `PullRequestHandler` depend on `scm.Provider` (capability interfaces: `CatalogReader`, `ChangeRequestReader`, `ChangeRequestWriter`, `ChangeRequestReviewer`, `ChangeRequestReviewReader`, `IdentityReader`, `IssueReader`, `IssueWriter`, `ContributorReader`, `WebhookRegistrar`, `CloneAuthorizer`), never on a forge client directly. `scm.For(repoType, creds)` dispatches on the provider the repository **URL** resolves to. Honouring that dispatch is load-bearing: the project path is not unique across forges, so a GitLab path run through the GitHub client silently imports an unrelated project's data. GitHub and GitLab (gitlab.com) are implemented; Gitea still returns `ErrUnsupportedProvider`, recorded on `sync_status`/`sync_error`.
...
- **Provider refusals are 409, outages are 503**: `scm.ErrProviderRejected` (and the more specific `scm.ErrSelfReview`) mark a failure the host understood and refused on its own rules. `scm.ProviderError` carries the upstream status, because whether a retry could ever help is a function of that status and nothing else in the system knows it. The clients no longer format failures into opaque strings — `github.APIError` / `gitlab.APIError` preserve the status, and anything below 500 is a refusal. Two consequences worth knowing: GitHub's 403 only maps to `ErrRateLimited` when the response says so (`x-ratelimit-remaining: 0` or `retry-after`), since it uses the same status for "your token may not do that"; and GitLab still folds 401 into `ErrUnauthorized`, so its one refusal worth naming is not classified yet (see FOLLOWUPS.md). `providerError`'s default no longer echoes `err.Error()`.
- **Not every provider has every action**: `scm.ErrUnsupportedCapability` distinguishes "this host cannot do that" from "that failed", and the handler maps it to `501`. `RequestChanges` on GitLab is the case it exists for — GitLab exposes approve/unapprove over REST but no reviewer "requested changes" state that is stable across self-hosted versions. Clients should hide the affordance rather than offer a button guaranteed to fail.
- **GitLab specifics that are easy to get wrong**: issues are addressed by `iid` like merge requests, and their state is `opened`, not `open`; merge requests are addressed by `iid`, not `id`; `/languages` returns percentages, not byte counts; the recursive tree is paginated with no `truncated` flag, so `gitlab.Client.GetTree` synthesizes one at a 30-page ceiling (a path missing from a truncated tree proves nothing — `internal/detect` treats it as `unknown`); `changes_count` is a string and can be `"1000+"`; per-file line counts do not exist and are counted from the diff body. Nested groups are real project paths, so `utils.ParseRepositoryURL` keeps every GitLab path segment (`group/subgroup/project`) while GitHub stays `owner/repo`.
...
- **Repository cache invalidation**: `RepositoryService.GetRepository` caches the response for an hour, so anything that changes a repository row must drop `redis.RepoKey(id)`. `SyncService.persist` exists for exactly that — every sync status write goes through it. Invalidating only on success is the bug that shipped: a repository read while `idle` reported `idle` for an hour after its sync failed, showing "never synced" instead of "sync failed".
- **GORM `default:` tags swallow meaningful zero values**: with `gorm:"default:X"` on a field, GORM treats the Go zero value as "unset" and omits it from the INSERT, so the column default wins. `OnboardingStep.IsRequired` had `default:true` and every step created as optional came back required — caught by the E2E suite, invisible to unit tests using a fake store. The tag is gone; the column keeps its default for hand-written SQL. Two latent instances remain (`Organization.IsActive`, `OrganizationMember.IsActive`, both `default:true`); nothing creates them as `false` today, so they are untouched — but do not add a `default:` tag to a boolean or number whose zero value means something.
- **Onboarding steps must point at live entities on save**: `ReplaceSteps` validates every reference against the caller's organization, so a flow holding a step whose repository was deleted cannot be saved until that step is removed. Reading it is fine — the runner renders `unavailable` — but writing it back is refused, because the same check that keeps a foreign reference out has no way to tell a dead reference from a wrong one.
