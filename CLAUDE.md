...
Backend of an Identity Provider (IDP) platform that integrates AI for code analysis. Provides JWT-based authentication, OAuth integration (GitHub/GitLab), repository management, AI dependency tracking, and semantic code search powered by embeddings.
...
- **AI Integration**: Anthropic API (Claude for code and dependency analysis)
...
internal/
├── api/
│   ├── handlers/         # HTTP request handlers (auth, repository, webhook, analysis, dependencies)
...
├── workers/              # asynq task handlers (SyncWorker, WebhookProcessor, AnalysisWorker, EmbeddingWorker, DependencyWorker)
├── storage/
...
- `ANTHROPIC_API_KEY`, `GITHUB_TOKEN`: External API keys; `GITHUB_TOKEN` is also used for private repository clones during metrics, dependency scans, and embedding generation.
