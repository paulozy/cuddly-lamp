...
Backend of an Identity Provider (IDP) platform that integrates AI for code analysis and code scaffolding. Provides JWT-based authentication, OAuth integration (GitHub/GitLab), repository management, AI dependency tracking, intelligent code templates, and semantic code search powered by embeddings.
...
- **AI Integration**: Anthropic API (Claude for code analysis, dependency analysis, and template generation)
...
internal/
├── api/
│   ├── handlers/         # HTTP request handlers (auth, repository, webhook, analysis, dependencies, templates)
...
├── workers/              # asynq task handlers (SyncWorker, WebhookProcessor, AnalysisWorker, EmbeddingWorker, DependencyWorker, TemplateWorker)
├── storage/
...
- **Architecture**: Pluggable `ai.Analyzer` and `ai.Generator` interfaces in `internal/ai/` — extensible to any LLM provider (Anthropic, OpenAI, Gemini, etc.)
- **Current Implementation**: Anthropic (Claude) via `internal/integrations/anthropic/` — Anthropic SDK with structured prompts
- **Swapping Providers**: Create new struct implementing `ai.Analyzer` and/or `ai.Generator`, then update the worker factory that calls `anthropic.NewClient()`
...
- `ANTHROPIC_API_KEY`, `GITHUB_TOKEN`: External API keys; Anthropic is used for analysis, dependency scanning, and template generation. `GITHUB_TOKEN` is also used for private repository clones during metrics, dependency scans, and embedding generation.
...
**Status**: ✅ All core features complete — Auth + Repo Sync + Webhook Pipeline + Encryption + AI Integration + Semantic Search + Dependency Tracking
